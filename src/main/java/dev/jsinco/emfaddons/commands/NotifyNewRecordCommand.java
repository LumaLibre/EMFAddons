...
public class NotifyNewRecordCommand implements CommandExecutor {
    ...
    public boolean onCommand(@NotNull CommandSender sender, @NotNull Command command, @NotNull String label, @NotNull String[] args) {
        ...
        if (player.hasPermission("emfaddons.notifyrecord")) {
            ...
            player.sendMessage(Util.colorcode(Util.PREFIX + "You will no longer be notified of aquatic journal records"));
        } else {
            luckPerms.getUserManager().modifyUser(player.getUniqueId(), user -> user.data().add(node));
            player.sendMessage(Util.colorcode(Util.PREFIX + "You will now be notified of aquatic journal records"));
        }
